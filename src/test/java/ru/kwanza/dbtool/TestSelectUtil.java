...
public abstract class TestSelectUtil extends DBTestCase {
    ...
    public static final String SELECT_SQL_6 =
            "SELECT xkey FROM test_table WHERE xkey IN(?) " +
                    " UNION ALL" +
                    " SELECT xkey FROM test_table WHERE xkey IN(?)";

    public static final String SELECT_SQL_7 =
    ...
    public void testEmpty() throws Exception {
        DatabaseOperation.CLEAN_INSERT.execute(getConnection(),
                new FlatXmlDataSetBuilder().build(this.getClass().getResourceAsStream("./data/empty.xml")));

        List<Integer> list = Arrays.asList(Integer.valueOf(1), Integer.valueOf(2), Integer.valueOf(3));
        List<TestEntity> result = getDBTool().selectList("SELECT xkey,name,version FROM test_table WHERE xkey IN(?) order by xkey", LIST_ROW_MAPPER, list
        );
    ...
    public void testSelect_2() throws Exception {
        ...
        Map<String, List<TestEntity>> mapList = getDBTool().selectMapList("SELECT xkey,name,version FROM test_table WHERE xkey IN(?) AND name=? order by xkey", MAP_ROW_MAPPER, keys, "n_0");
    ...
    public void testSelect_NoResult() throws Exception {
        ...
        Map<String, List<TestEntity>> mapList = getDBTool().selectMapList(SELECT_SQL_4, MAP_ROW_MAPPER,
                keys, "test");
    ...
    public void testSelect_WithSQLParameter() throws Exception {
        ...
        List<TestEntity> result = getDBTool().selectList(SELECT_SQL_2, LIST_ROW_MAPPER, keys,
                new SqlParameterValue(Types.VARCHAR, "n_0"));
    ...
    public void testSelect_MapOfMap_By_Collections() throws Exception {
        ...
        Map<String, Map<Integer, TestEntity>> map =
                getDBTool().selectMapOfMaps(SELECT_SQL_8, MAP_OF_MAP_ROW_MAPPER, names, keys);
