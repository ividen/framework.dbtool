...
public abstract class TestUpdateUtilWithOptimistic extends AbstractTestUpdateUtil {
    ...
    public void testSuccessUpdate() throws Throwable {
        ...
        try {
            List<TestEntity> testEntities = getDBTool().selectList("select xkey,name,version from test_table",
                    LIST_ROW_MAPPER);

            try {
                assertEquals(11, getDBTool().update("update test_table set version=? ,name=? where xkey=? and version=?",
                        testEntities, TEST_BATCHER_6,
    ...
    public void testUpdateConstrained_1() throws Throwable {
        ...
        try {
            ...
            try {
                assertEquals(10, getDBTool().update("update test_table set version=? ,name=? where xkey=? and version=?",
                        testEntities, TEST_BATCHER_6,
                        "select xkey,version from test_table where xkey in(?)", KEY_VERSION_MAPPER, TestEntity.KEY, TestEntity.VERSION));
    ...
    public void testUpdateConstrained_3() throws Throwable {
        ...
        try {
            ...
            try {
                assertEquals(9, getDBTool().update("update test_table set version=? ,name=? where xkey=? and version=?",
                        testEntities, TEST_BATCHER_6,
                        "select xkey,version from test_table where xkey in(?)", KEY_VERSION_MAPPER, TestEntity.KEY, TestEntity.VERSION));
                fail("Must throw Exception!");
            } catch (UpdateException e) {
    ...
    public void testUpdateConstrained_6() throws Throwable {
        ...
        try {
            ...
            try {
                assertEquals(0, getDBTool().update("update test_table set version=? ,name=? where xkey=? and version=?",
                        testEntities, TEST_BATCHER_6,
                        "select xkey,version from test_table where xkey in(?)", KEY_VERSION_MAPPER, TestEntity.KEY, TestEntity.VERSION));
                fail("Must throw Exception!");
            } catch (UpdateException e) {
    ...
    public void testUpdate_Skip_1() throws Throwable {
        ...
        try {
            ...
            try {
                assertEquals(10, getDBTool().update("update test_table set version=? ,name=? where xkey=? and version=?",
                        testEntities, new TestEntityUpdateSetter5(testEntities, 0),
                        "select xkey,version from test_table where xkey in(?)", KEY_VERSION_MAPPER, TestEntity.KEY, TestEntity.VERSION));
        ...
        try {
            ...
            try {
                assertEquals(10, getDBTool().update("update test_table set version=? ,name=? where xkey=? and version=?", testEntities,
                        new TestEntityUpdateSetter5(testEntities, 10),
                        "select xkey,version from test_table where xkey in(?)", KEY_VERSION_MAPPER, TestEntity.KEY, TestEntity.VERSION));
    ...
    public void testUpdate_Skip_3() throws Throwable {
        ...
        try {
            ...
            try {
                assertEquals(10, getDBTool().update("update test_table set version=? ,name=? where xkey=? and version=?", testEntities,
                        new TestEntityUpdateSetter5(testEntities, 5),
                        "select xkey,version from test_table where xkey in(?)", KEY_VERSION_MAPPER, TestEntity.KEY, TestEntity.VERSION));
        ...
        try {
            ...
            try {
                assertEquals(9, getDBTool().update("update test_table set version=? ,name=? where xkey=? and version=?", testEntities,
                        new TestEntityUpdateSetter5(testEntities, 0, 10),
                        "select xkey,version from test_table where xkey in(?)", KEY_VERSION_MAPPER, TestEntity.KEY, TestEntity.VERSION));
        ...
        try {
            ...
            try {
                assertEquals(8, getDBTool().update("update test_table set version=? ,name=? where xkey=? and version=?", testEntities,
                        new TestEntityUpdateSetter5(testEntities, 0, 5, 10),
                        "select xkey,version from test_table where xkey in(?)", KEY_VERSION_MAPPER, TestEntity.KEY, TestEntity.VERSION));
        ...
        try {
            ...
            try {
                assertEquals(7, getDBTool().update("update test_table set version=? ,name=? where xkey=? and version=?", testEntities,
                        new TestEntityUpdateSetter5(testEntities, 1, 5, 6, 9),
                        "select xkey,version from test_table where xkey in(?)", KEY_VERSION_MAPPER, TestEntity.KEY, TestEntity.VERSION));
    ...
    public void testUpdateOptimistic_1() throws Throwable {
        ...
        try {
            ...
            testEntities = getDBTool().selectList("select xkey,name,version from test_table order by xkey",
                    LIST_ROW_MAPPER);
            getTxManager().commit(status);
        } catch (Throwable e) {
            getTxManager().rollback(status);
        ...
        try {
            assertEquals(1, getDBTool().getJdbcTemplate().update("update test_table set version=version+2, name=? where xkey=? and version=?",
                    optimistic.getName(), optimistic.getKey(), optimistic.getVersion()));
            getTxManager().commit(status);
        } catch (Throwable e) {
            getTxManager().rollback(status);
            throw e;
    ...
    public void testUpdateOptimistic_4() throws Throwable {
        ...
        try {
            assertEquals(1, getDBTool().getJdbcTemplate().update("update test_table set version=version+2, name=? where xkey=? and version=?",
                    optimistic1.getName(), optimistic1.getKey(), optimistic1.getVersion()));
            assertEquals(1, getDBTool().getJdbcTemplate().update("update test_table set version=version+2, name=? where xkey=? and version=?",
                    optimistic2.getName(), optimistic2.getKey(), optimistic2.getVersion()));
            assertEquals(1, getDBTool().getJdbcTemplate().update("update test_table set version=version+2, name=? where xkey=? and version=?",
                    optimistic3.getName(), optimistic3.getKey(), optimistic3.getVersion()));
            getTxManager().commit(status);
        } catch (Throwable e) {
            getTxManager().rollback(status);
            throw e;
