...
public abstract class StatementImpl<T> implements IStatement<T> {
    ...
    private abstract class BaseExtractor<TYPE> implements ResultSetExtractor {
        ...
        private boolean hasIdValue(QueryEntityInfo queryEntityInfo, ResultSet rs, Class entityClass) throws SQLException {
            ...
            if (FieldValueExtractor.getValue(rs, Column.getFullColumnName(queryEntityInfo, idField), idField.getProperty().getType()) == null) {
                return false;
            }
            return true;
        }
        ...
        private void readAndFill(ResultSet rs, Collection<IFieldMapping> fields, QueryEntityInfo queryEntityInfo, Object obj) throws SQLException {
            for (IFieldMapping idf : fields) {
                Object value = FieldValueExtractor.getValue(rs, Column.getFullColumnName(queryEntityInfo, idf), idf.getProperty().getType());
